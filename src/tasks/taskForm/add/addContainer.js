...
export default function AddTaskContainer(props) {
          ...
          <IconButton
            onPress={() => {
              if (cannotSave) {
                ...
                addTaskFunc({ title, important, title, closeDate, assignedTo, company, deadline, description, pendingDate, project, requester, status, tags, subtasks, materials, customAttributes, attachments });
              }
            }}
            variant="ghost"
            _icon={{
  ...
  }, [navigation, important, title, closeDate, assignedTo, company, deadline, description, pendingDate, project, requester, status, tags, subtasks, materials, customAttributes, attachments, cannotSave]);
      ...
      if (updatedProject) {
        ...
        updatedProject.addCustomAttributes.forEach((item, i) => {
          const value = {
            ...
            selectValues: item.selectValues.filter((value) => value.def).map((value) => ({
              ...value,
              label: value.value.substring(0, 1).toUpperCase() + value.value.substring(1)
            })),
          };
          const selectValues = item.selectValues.map((value) => ({
            ...value,
            label: value.value.substring(0, 1).toUpperCase() + value.value.substring(1)
          }));
